...
class ConvBlock(nn.Sequential):
    def __init__(self, in_channels: int, out_channels: int, kernel_size: int = 3, stride: int = 1, prelu: bool = False):
        pad = kernel_size // 2
...
class MFAttentionBlock(nn.Module):
    ...
    def __init__(self, channels: int = 8, patch_size: int = 8, heads: int = 8):
        ...
        self.swir_enc = nn.Sequential(ConvBlock(channels, channels // 2, prelu=True), ConvBlock(channels // 2, channels // 2, prelu=True))
        self.lwir_enc = nn.Sequential(ConvBlock(channels, channels // 2, prelu=True), ConvBlock(channels // 2, channels // 2, prelu=True))
        ...
        self.ffn_s = nn.Sequential(nn.Linear(embed_dim, embed_dim * 2), nn.ReLU(inplace=True), nn.Linear(embed_dim * 2, embed_dim))
        self.ffn_l = nn.Sequential(nn.Linear(embed_dim, embed_dim * 2), nn.ReLU(inplace=True), nn.Linear(embed_dim * 2, embed_dim))
    ...
    def forward(self, swir: torch.Tensor, lwir: torch.Tensor) -> torch.Tensor:
        ...
        l = self.lwir_enc(lwir)

        s_tok = self.embed_s(s)
        l_tok = self.embed_l(l)

        s_attn, _ = self.self_s(self.norm1s(s_tok), self.norm1s(s_tok), self.norm1s(s_tok), need_weights=False)
        l_attn, _ = self.self_l(self.norm1l(l_tok), self.norm1l(l_tok), self.norm1l(l_tok), need_weights=False)
        ...
        s_tok = s_tok + self.ffn_s(s_cross)
        l_tok = l_tok + self.ffn_l(l_cross)
